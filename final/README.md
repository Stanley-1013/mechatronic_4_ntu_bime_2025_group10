...
| 版本 | 狀態 | 架構 | 主要特色 | 檔案位置 |
|------|------|------|--------|---------|
| **v5_competition** | 🏆 實戰版 | Arduino 自主控制 | **競賽驗證 113/120** | [`v5_competition/`](./v5_competition/) |
| **v3_stable** | ✅ 穩定版 | Arduino 自主控制 | 雙超聲波沿牆、PD 參數、角落擺頭 | [`v3_stable/`](./v3_stable/) |
| **v4_simple** | 🧪 對比版 | 簡化超聲波方案 | 防突波濾波、快速原型 | [`v4_simple/`](./v4_simple/) |
| **v2_autonomous** | 📚 參考版 | Pi 決策 + Arduino 執行 | IMU PID 控制、紅色偵測 | [`v2_autonomous/`](./v2_autonomous/) |

### 版本選擇指南

#### 🏆 首推：v5_competition（競賽實戰版）
- **成績：** 113 / 120（94.2%）
- **架構：** 延續 v4 簡化架構，參考 v3 調參經驗
- **特色：**
  - 純 Arduino 控制（無需 Pi 即時介入）
  - 固定入口動作 + 優化角落策略
  - 競賽現場驗證通過
- **適用：** 正式競賽、快速部署
- **穩定性：** ⭐⭐⭐⭐⭐
